...
static void tcp_server_task(void *pvParameters) {
	...
	if (setsockopt(listen_sock, SOL_SOCKET, SO_RCVTIMEO,
			(char*) &timeout_listen, sizeof(timeout_listen)) == 0) {
...
void bus_rs485(void *arg) {
	...
	while (!esp_mesh_is_root()) {
		...
		if (txctrl > 0) {
			printf("Tx FIFO:%u \r\n", txctrl);
			ctrl_cola = xQueueReceive(RxRS485, (uint8_t* )tx_buf + 4,
					pdMS_TO_TICKS(500));
			if (ctrl_cola == pdTRUE) {
				tx_buf[0] = dataTx[0];
				tx_buf[1] = dataTx[1];
				...
				esp_err_t err = esp_mesh_send(NULL, &dataMesh, MESH_DATA_P2P,
						NULL, 0);
				if (err == ESP_OK) {
					printf("Mandado\r\n");
				}
...
esp_err_t activa_entrada(char *partition_name, char *page_namespace,
		char **entry_key, int32_t *actual_entry_index, int32_t *Pulsos) {
...
esp_err_t leer_pagina_variable(char **pname, char **pvActual,
		int32_t *ContadorEntradaActual, char **regActualPaginaVariable,
	...
	if (entradasEnPv == 0)
		entradasEnPv = 1; //En caso de no encontrar nada en la página actual se asigna 1 para crear la entrada e1.
	...
	*ContadorEntradaActual = entradasEnPv;
	...
	//Abriendo la página variable
	esp_err_t err = nvs_open_from_partition(*pname, *pvActual, NVS_READWRITE,
			&my_handle);
	if (err != ESP_OK)
		return err;
...
void modbus_tcpip_pulsos(void *arg) {
	...
	while (!esp_mesh_is_root()) {
		...
		ESP_LOGI("MODBUS RX", "%x %x %x %x %x %x %x %x %x %x %x %x", rx_buf[0],
				...
				rx_buf[6], rx_buf[7], rx_buf[8], rx_buf[9], rx_buf[10],
				rx_buf[11]);
...
static void rotar_nvs(void *arg) {
	...
	if (err != ESP_OK)
		printf("Error (%s) buscando el registro escrito más reciente\n",
				esp_err_to_name(err));
	ESP_LOGI("LPV",
			"pvActual: %s indexEntradaActual: %d Entrada actual: %s Cuenta pulsos: %d",
			...
			if (gpio_get_level(GPIO_NUM_0) == 0) {
				...
				if (indexPv == Limite_paginas_por_particion
					...
					if (partition_number <= max_particiones) {
						...
						ESP_LOGW("CNP", "Partition changed to %s",
								partition_name);
						nvs_flash_init_partition(partition_name);

						ESP_LOGI("PARTICION ACTUAL", "DEBUG 2 %s",
								partition_name);

						//Llenando particion
						esp_err_t err = nvs_open_from_partition(partition_name,
								"storage", NVS_READWRITE, &my_handle);
						if (err != ESP_OK)
							ESP_LOGE("NVS", "ERROR IN NVS_OPEN");
						//		free(*pname);
						...
						//Get del número de la partición
						err = nvs_get_u8(my_handle, "pnumber",
								&partition_number);
						if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND)
							ESP_LOGE("NVS", "ERROR IN GET");
						else if (err == ESP_ERR_NVS_NOT_FOUND) {
							err = nvs_set_u8(my_handle, "pnumber",
									partition_number);
							if (err != ESP_OK)
								ESP_LOGE("NVS", "ERROR IN SET");
							err = nvs_commit(my_handle);
				...
				if (partition_number <= max_particiones) {
					err = contar_pulsos_nvs(&partition_name, &partition_number,
							&indexPv, &indexEntradaActual, &cuentaPulsosPv);

					total_pulsos = inicial_pulsos
							+ //Aporte de los pulsos iniciales
							(partition_number - 1)
									* Limite_paginas_por_particion
									* Limite_entradas_por_pagina
									* Limite_pulsos_por_entrada
							+ //Aporte c/u de las particiones
							(indexPv - 1) * Limite_entradas_por_pagina
									* Limite_pulsos_por_entrada
							+ //Aporte de las paginas anteriores
							(indexEntradaActual - 1) * Limite_pulsos_por_entrada
							+ //Aporte de la página actual
							cuentaPulsosPv;	//Aporte de la entrada actual

					ESP_LOGI("PULSOS",
							"PARTICION: %d PV: %d ENT: %d Pulsos: %d",
	...
	case MESH_EVENT_PARENT_CONNECTED: {
		...
		if (esp_mesh_is_root()) {
			...
			case (rs485):
				...
				if (creador) {
					xTaskCreatePinnedToCore(esp_mesh_p2p_rx_main, rx_child,
							3072 * 2, NULL, 5, NULL, 0);
				}
				creador = vTaskB(rx_rs485);
				if (creador) {
					xTaskCreatePinnedToCore(bus_rs485, rx_rs485, 3072 * 3, NULL,
							5, NULL, 1);
			...
			case (pulsos):
				...
				if (creador)
					xTaskCreatePinnedToCore(rotar_nvs, count_pulse, 4 * 1024,
							NULL, 5, NULL, 1);
...
void ip_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id,
	...
	if (creador) {
		xTaskCreatePinnedToCore(tcp_server_task, "tcp_server", 1024 * 4, NULL,
				1, NULL, 1);
	}
	creador = vTaskB(tx_root);
	if (creador) {
		xTaskCreatePinnedToCore(esp_mesh_p2p_tx_main, "TX", 1024 * 3, NULL, 5,
				NULL, 0);
	}
