...
//					muestra_energy =
//(float)pulsos/(float)fconv; 					err =
// nvs_set_u64(ctrl_pulsos, "energy", pulsos);
// printf((err != ESP_OK) ? "Error pulse counter set!\n" : "Set Done energy %.6f
// kWh\n Pulsos:
// %llu\n",muestra_energy,pulsos); 					err =
// nvs_commit(ctrl_pulsos); 					printf((err !=
// ESP_OK) ? "Error in pulse counter commit!\n" : "Commit Done\n");
// close(ctrl_pulsos); 					ESP_LOGI(MESH_TAG,"Stop
// Timer y Reinicio desde guardado en NVS"); aux = pulsos;
//				}
//			}
//			}
//			ESP_LOGE(MESH_TAG,"La tarea de guardado en flash de
// pulsos fue eliminada"); 			vTaskDelete(NULL);
//}
...
// energy_ini, (int)muestra_energia);
//
//	while(!esp_mesh_is_root()){
