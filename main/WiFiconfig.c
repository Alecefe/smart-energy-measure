...
static void tg0_timer_init(int timer_idx, bool auto_reload,
...
void supervisor_sta(void* arg) {
  while (1) {
    ...
    config_wifi_ap();
    ESP_LOGE("Supervisor",
             "Eliminada la tarea de supervisión cambiando a modo AP");
    vTaskDelete(NULL);
  }
}
...
static void ManejadorEventos(void* arg, esp_event_base_t event_base,
  ...
  switch (event_id) {
    ...
    case WIFI_EVENT_AP_STACONNECTED: {
      ...
      ESP_LOGI("SoftAP", "station " MACSTR " join, AID=%d", MAC2STR(event->mac),
               event->aid);
    } break;
...
void config_wifi_sta(void) {
  ...
  wifi_config_t wifi_config_sta = {
      .sta = {.ssid = nombreWiFi, .password = claveWiFi},
  };
