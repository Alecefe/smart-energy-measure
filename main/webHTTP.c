...
bool ahora = true;
conversion_t conversion[4] = {
    {rs485, "rs485"}, {pulsos, "pulsos"}, {chino, "chino"}, {enlace, "enlace"}};
...
tipo_de_medidor str2enum(const char *str) {
  ...
  for (j = 0; j < sizeof(conversion) / sizeof(conversion[0]); ++j)
    if (!strcmp(str, conversion[j].str)) return conversion[j].val;
...
void set_form_flash_mesh(form_mesh form) {
  ...
  if (err != ESP_OK) {
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash max layer!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash max STA!\n\r", esp_err_to_name(err));
    }

    ESP_LOGI("SET_MESH", "%d", form.port);
    err = nvs_set_u16(ctrl_mesh, "port", form.port);
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash port!\n\r", esp_err_to_name(err));
    }

    err = nvs_commit(ctrl_mesh);
    if (err != ESP_OK) {
      printf("Error (%s) while the commit stage!\n\r", esp_err_to_name(err));
    }
  }
  nvs_close(ctrl_mesh);
}
...
void set_form_flash_modbus(form_modbus form) {
  ...
  if (err != ESP_OK) {
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash meter type!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash slave ID!\n\r", esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash conv. factor!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash Baud Rate!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash Initial Energy!\n\r",
             esp_err_to_name(err));
    }

    err = nvs_commit(ctrl_modbus);
    if (err != ESP_OK) {
      ESP_LOGW("NVS_COMMIT", "Error(%s)", esp_err_to_name(err));
    }
  }
...
void set_form_flash_login(form_login form) {
  ...
  if (err != ESP_OK) {
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash user login!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash user password!\n\r",
             esp_err_to_name(err));
    }
...
void set_form_flash_locwifi(form_locwifi form) {
  ...
  if (err != ESP_OK) {
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash SSID!\n\r", esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash WiFi Pass!\n\r",
             esp_err_to_name(err));
    }
...
void set_form_flash_mqtt(form_mqtt form) {
  ...
  if (err != ESP_OK) {
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash MQTT port!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash MQTT Pub. Topic!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash advanced config!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash URI!\n\r", esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash Type!\n\r", esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash app layer!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash MQTT User!\n\r",
             esp_err_to_name(err));
    }
    ...
    if (err != ESP_OK) {
      printf("Error (%s) setting in flash MQTT Pass!\n\r",
             esp_err_to_name(err));
    }
...
void get_form_flash_modbus(form_modbus *form) {
  ...
  if (err != ESP_OK) {
    ...
    switch (err) {
      ...
      case ESP_ERR_NVS_NOT_FOUND:
        ESP_LOGI(nvs_tag,
                 "No se encontró slave ID, se tomará el valor por defecto 1");
        form->energia = 1;
        break;
      default:
        printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
    ...
    switch (err) {
      ...
      case ESP_ERR_NVS_NOT_FOUND:
        ESP_LOGI(
            nvs_tag,
            "No se encontró factor de conversion, asumiendo 1200 [imp/kWh]");
        form->energia = 1200;
        break;
      default:
        printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
...
void get_form_flash_mqtt(form_mqtt *form) {
  ...
  if (err != ESP_OK) {
    ...
    switch (err) {
      case ESP_OK:
        if (form->advance == 1) {
          ESP_LOGI(nvs_tag, "%s", "Advanced configuration");
        } else {
          ESP_LOGI(nvs_tag, "%s", "Basic configuration");
        }
        break;
      case ESP_ERR_NVS_NOT_FOUND:
    ...
    if (form->advance != 1) {  // Configuración básica
      ...
      switch (err) {
        case ESP_OK:
          if (form->type == 1) {
            ESP_LOGI(nvs_tag, "%s", "Public Broker");
          } else {
            ESP_LOGI(nvs_tag, "%s", "Private Broker");
          }
          break;
        case ESP_ERR_NVS_NOT_FOUND:
...
bool fill_form_mesh(char *p, form_mesh *form) {
  ...
  for (int i = 0; i < 17; i++) {
    if ((i == 2 || i == 5 || i == 8 || i == 11 || i == 14) &&
      ...
      return false;
    } else if (!((aux_meshID[i] >= 0x30 && aux_meshID[i] <= 0x39) ||
                 (aux_meshID[i] >= 0x61 || aux_meshID[i] <= 0x66))) {
      printf("Mesh ID No válida caractér no válido [%d]", i);
      return false;
    }
  }
  for (int i = 0; i < 18; i++) {
    if (aux_meshID[i] > 0x60) {
      aux_meshID[i] -= 0x57;
    }
    if (aux_meshID[i + 1] > 0x60) {
      aux_meshID[i + 1] -= 0x57;
    }
    form->mesh_id[i / 3] =
        ((0x0f & aux_meshID[i]) << 4) | (0x0f & aux_meshID[i + 1]);
    i += 2;
  }
  ...
  if (strlen(aux_mesh_password) < 6 || strlen(aux_mesh_password) > 15) {
    ...
    for (int i = 0; i <= strlen(aux_mesh_password); i++) {
      form->meshappass[i] = aux_mesh_password[i];
    }
  }
  ESP_LOGW("FILL MESH", "Mesh access password: %s", form->meshappass);

  /*Max Layer*/
  uint8_t aux_max_layer = (uint8_t)strtoul(
      cJSON_GetObjectItem(root, "maxlayer")->valuestring, &eptr, 10);
  if (aux_max_layer >= 1 && aux_max_layer <= 25) {
    form->max_layer = aux_max_layer;
  } else {
    return false;
  }
  ...
  /*Max sta*/
  uint8_t aux_max_sta = (uint8_t)strtoul(
      cJSON_GetObjectItem(root, "maxsta")->valuestring, &eptr, 10);
  if (aux_max_sta >= 1 && aux_max_sta <= 9) {
    form->max_sta = aux_max_sta;
  } else {
    return false;
  }
  ...
  /*Port*/
  uint32_t aux_port = (uint32_t)strtoul(
      cJSON_GetObjectItem(root, "port")->valuestring, &eptr, 10);
  if (aux_port <= 65535) {
    form->port = (uint16_t)aux_port;
  } else {
    return false;
  }
...
bool fill_form_modbus(char *p, form_modbus *form) {
  ...
  if (cJSON_IsNull(cJSON_GetObjectItem(root, "baudrate")) == 0) {
    uint32_t aux_baud_rate = (uint32_t)strtoul(
        cJSON_GetObjectItem(root, "baudrate")->valuestring, &eptr, 10);
    if (aux_baud_rate >= 1 && aux_baud_rate <= 19200) {
      form->baud_rate = aux_baud_rate;
    } else {
      return false;
    }
  ...
  if (cJSON_IsNull(cJSON_GetObjectItem(root, "convfac")) == 0) {
    uint16_t aux_conversion = (uint16_t)strtoul(
        cJSON_GetObjectItem(root, "convfac")->valuestring, &eptr, 10);
    if (aux_conversion >= 1) {
      form->conversion = aux_conversion;
    } else {
      return false;
    }
  ...
  if (cJSON_IsNull(cJSON_GetObjectItem(root, "iniene")) == 0) {
    uint64_t aux_iniene = (uint64_t)strtoul(
        cJSON_GetObjectItem(root, "iniene")->valuestring, &eptr, 10);
    if (aux_iniene >= 0) {
      form->energia = aux_iniene;
    } else {
      return false;
    }
  ...
  if (cJSON_IsNull(cJSON_GetObjectItem(root, "slaveid")) == 0) {
    uint8_t aux_slaveid = (uint8_t)strtoul(
        cJSON_GetObjectItem(root, "slaveid")->valuestring, &eptr, 10);
    if (aux_slaveid >= 1 && aux_slaveid <= 254) {
      form->slaveid = aux_slaveid;
    } else {
      return false;
    }
...
bool fill_form_locwifi(char *p, form_locwifi *form) {
  ...
  if (strlen(aux_ssid) < 8 || strlen(aux_ssid) > 30) {
    ...
    for (int i = 0; i <= strlen(aux_ssid); i++) {
      form->ssid[i] = aux_ssid[i];
    }
  }
  ...
  if (strlen(aux_ssid) < 8 || strlen(aux_ssid) > 30) {
    ...
    for (int i = 0; i <= strlen(aux_pass); i++) {
      form->password[i] = aux_pass[i];
    }
  }
...
bool fill_form_login(char *p, form_login *form) {
  ...
  if (strlen(aux_user) < 6 || strlen(aux_user) > 15) {
    ...
    for (int i = 0; i <= strlen(aux_user); i++) {
      form->user[i] = aux_user[i];
    }
  }
...
bool fill_form_mqtt(char *p, form_mqtt *form) {
  ...
  /*Advance*/
  uint8_t aux_advance = (uint8_t)strtoul(
      cJSON_GetObjectItem(root, "advance")->valuestring, &eptr, 10);
  if (aux_advance == 1 || aux_advance == 0) {
    form->advance = aux_advance;
  } else {
    return false;
  }
  ESP_LOGW(
      "FILL MQTT", "%s",
      (form->advance == 1) ? "Advanced settings ON" : "Advanced settings OFF");
  ...
  if (strlen(aux_uri) > 50) {
    ...
    for (int i = 0; i <= strlen(aux_uri); i++) {
      form->uri[i] = aux_uri[i];
    }
  }
  ...
  /*Port*/
  uint16_t aux_port = (uint16_t)strtoul(
      cJSON_GetObjectItem(root, "port")->valuestring, &eptr, 10);
  if (aux_port > 0) {
    form->port = aux_port;
  } else {
  ...
  if (strlen(aux_pubtopic) > 20) {
    ...
    for (int i = 0; i <= strlen(aux_pubtopic); i++) {
      form->pubtopic[i] = aux_pubtopic[i];
    }
  }
  ESP_LOGW("FILL MQTT", "Publication topic: %s", form->pubtopic);

  /*Type*/
  uint8_t aux_type = (uint8_t)strtoul(
      cJSON_GetObjectItem(root, "type")->valuestring, &eptr, 10);
  if (aux_type == 1 || aux_type == 0) {
    form->type = aux_type;
  } else {
    return false;
  }
  ESP_LOGW(
      "FILL MQTT", "%s",
      (form->type == 1) ? "Type of broker: Public" : "Type of broker: Private");
  ...
  if (strlen(aux_password) > 20) {
    ...
    for (int i = 0; i <= strlen(aux_password); i++) {
      form->password[i] = aux_password[i];
    }
  }
  ESP_LOGW("FILL MQTT", "MQTT Password: %s", form->password);

  /*App Layer*/
  uint8_t aux_applayer = (uint8_t)strtoul(
      cJSON_GetObjectItem(root, "app_layer")->valuestring, &eptr, 10);
  if (aux_applayer == 1 || aux_applayer == 2 || aux_applayer == 3 ||
      aux_applayer == 4) {
    form->app_layer = aux_applayer;
  } else {
    return false;
  }
  ESP_LOGW(
      ...
      (form->advance == 1)
          ...
          : (form->advance == 2)
                ? "App Layer: SSL"
                : (form->advance == 3) ? "App Layer: WS" : "App Layer: WSS");
...
static esp_err_t login_post_handler(
    httpd_req_t
        *req) { /*Revisa que el usuario y contraseña del login sea el correcto*/
  ...
  if ((strlen(aux_usuario) == strlen(valid_user)) &&
      (strlen(aux_contrasena) == strlen(valid_pass))) {
    if ((strcmp(aux_usuario, valid_user) == 0) &&
        (strcmp(aux_contrasena, valid_pass) == 0)) {
...
static esp_err_t form_mesh_get_handler(
    httpd_req_t *req) { /*Toma los datos de la mesh en flash y los sube al
                           formulario mesh*/
...
static esp_err_t form_mesh_post_handler(
    httpd_req_t *req) { /*Recepción de formulario de configuración mesh*/
...
esp_err_t start_rest_server(const char *base_path) {
  ...
  httpd_config_t config = {.task_priority = tskIDLE_PRIORITY + 5,
                           .stack_size = 1024 * 25,
  ...
  httpd_uri_t mesh_get_uri = {.uri = "/mesh",
                              .method = HTTP_GET,
  ...
  httpd_uri_t mesh_post_uri = {.uri = "/mesh",
                               .method = HTTP_POST,
  ...
  httpd_uri_t modbus_get_uri = {.uri = "/modbus",
                                .method = HTTP_GET,
  ...
  httpd_uri_t modbus_post_uri = {.uri = "/modbus",
                                 .method = HTTP_POST,
  ...
  httpd_uri_t locwifi_get_uri = {.uri = "/locwifi",
                                 .method = HTTP_GET,
  ...
  httpd_uri_t locwifi_post_uri = {.uri = "/locwifi",
                                  .method = HTTP_POST,
  ...
  httpd_uri_t userlog_get_uri = {.uri = "/userlog",
                                 .method = HTTP_GET,
  ...
  httpd_uri_t mqtt_post_uri = {.uri = "/mqtt",
                               .method = HTTP_POST,
