...
void app_main() {
	...
	if (gpio_get_level(PIN_CONFIG_1) == 0) {
		...
		case (rs485):
			ESP_LOGW(MESH_INIT, "MESH METER BAUD RATE = %u",
					form_modbus.baud_rate);
			break;

		case (pulsos):
			ESP_LOGW(MESH_INIT, "MESH METER INITIAL ENERGY = %"PRIu64"pulsos",
					form_modbus.energia);
			ESP_LOGW(MESH_INIT, "MESH SLAVE ID = %d", form_modbus.slaveid);
			ESP_LOGW(MESH_INIT, "Conversion Factor: %d imp/kWh",
					form_modbus.conversion);
			break;

		case (chino):
			ESP_LOGW(MESH_INIT, "MESH METER BAUD RATE = %"PRIu32,
					form_modbus.baud_rate);
			break;
