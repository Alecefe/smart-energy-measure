...
void init_slaves(bool *slaves) {
  int i = 0;
  for (i = 0; i < 256; i++) slaves[i] = false;
  for (i = 1; i <= SLAVES; i++) slaves[i] = true;
}
...
int check_exceptions(uint8_t *data) {
  ...
  if (FUNCTION < 0x80) return ESP_OK;
  switch (EXCEPTION) {
    case ILLEGAL_FUNC:
    ...
    case ILLEGAL_DATA:
      ...
      break;
    case SLAVE_FAIL:
      ESP_LOGE(TAG, "Slave  %d fail attempting to execute the requested action",
               SLAVE);
      break;
    case ACK:
